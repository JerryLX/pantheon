...
def main():
    ...
    cc_repo2 = path.join(context.third_party_dir, 'autocc')
    #send_src = path.join(cc_repo, 'dagger', 'run_sender.py')
    #recv_src = path.join(cc_repo, 'env', 'run_receiver.py')
    send_src = path.join(cc_repo2, 'run_sender.py')
    recv_src = path.join(cc_repo2, 'gym_pantheon', 'envs', 'run_receiver.py')
